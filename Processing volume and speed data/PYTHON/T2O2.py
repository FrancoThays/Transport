...
output_file = pl.Path("..\RESULTADOS\objetivo_02.csv")
...
postos = [i[0] for i in cur.fetchall ()] #cur.fetchall() apaga o conteúdo da consulta!!!!
...
for posto in postos:
    ...
    while ts_fim_corrente <= ts_fim + datetime.timedelta(hours=1):
        ...
        sql_ini = ts_ini_corrente.strftime("%Y-%m-%d %H")
        #print (sql_ini)
        sql_fim = ts_fim_corrente.strftime ("%Y-%m-%d %H")
        ...
        ts_ini_corrente = ts_ini_corrente + datetime.timedelta (hours=1)
        ts_fim_corrente = ts_fim_corrente + datetime.timedelta(hours=1)
